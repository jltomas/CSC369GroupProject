...
object App {
  ...
  def findTopCategories():Unit = {
    val file = Source.fromFile("input/911.csv").getLines.toList
    val line = sc.parallelize(file)
    val lines = line.filter(x => x.split(",")(0).trim() != "lat")
  ...
  def findEmergencyByZipcode():Unit = {
    val line = Source.fromFile("input/911.csv").getLines.toList
    val lines = sc.parallelize(line)
    val myTuple = lines
    .map(_.split(","))
    .map(x => (x(3), x(4)))
    .persist()
